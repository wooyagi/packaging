...
사용법: /update {티커} (예: /update CAMT)

[파일명 규칙]
  ONTO  → onto-innovation.html / transcripts/onto-*.html
  KLAC  → kla.html / transcripts/klac-*.html
  CAMT  → camtek.html / transcripts/camt-*.html
  FORM  → formfactor.html / transcripts/form-*.html
  BESI  → besi.html / transcripts/besi-*.html
  (그 외: 회사명 소문자·하이픈 / transcripts/{티커소문자}-*.html)
