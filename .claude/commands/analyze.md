...
티커가 아닌 회사명 소문자 하이픈 형식으로 저장한다:
  ONTO   → onto-innovation.html
  KLAC   → kla.html
  CAMT   → camtek.html
  FORM   → formfactor.html
  TPRO   → technoprobe.html
  LRCX   → lam-research.html
  AMAT   → applied-materials.html
  6146   → disco.html
  8035   → tokyo-electron.html
  SMHN   → suss-microtec.html
  0522   → asmpt.html
  BESI   → besi.html
  ASML   → asml.html
  6857   → advantest.html
...
[비상장사 처리 규칙]
비상장(공시 데이터 없음) 기업은 재무 섹션을 생략하고
기술·뉴스·계약 동향 중심 페이지를 생성한다.
(첨단 패키징 밸류체인의 비상장 소재·장비·OSAT·팹리스 스타트업 등)
...
- 밸류에이션: PER, PBR, EV/EBITDA + 배당수익률 (한국 시장 기준)
- 수치 출처: DART 사업보고서·분기보고서 우선,
  KIND(kind.krx.co.kr), 네이버금융, 각사 IR 자료 교차 확인

[회사명으로 분석 가능한 목록 — 이름으로 입력 시 자동 매핑]
첨단 패키징 밸류체인 한국 상장사(예시, 필요 시 확장):
  한미반도체     → KRX 042700 → hanmi-semiconductor.html
  이오테크닉스   → KRX 039030 → eo-technics.html
  하나마이크론   → KRX 067310 → hana-micron.html
  SFA반도체      → KRX 036540 → sfa-semicon.html
  네패스         → KRX 033640 → nepes.html
  엘비세미콘     → KRX 061970 → lb-semicon.html
